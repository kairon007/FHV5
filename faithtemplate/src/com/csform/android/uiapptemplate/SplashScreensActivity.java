...
public class SplashScreensActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Thread timerThread = new Thread(){
			public void run(){
				try{
					...
					Intent intent = new Intent(SplashScreensActivity.this,LogInPageActivity.class);
		...
		String category = SPLASH_SCREEN_OPTION_2;
		Bundle extras = getIntent().getExtras();
		if (extras != null && extras.containsKey(SPLASH_SCREEN_OPTION)) {
			category = extras.getString(SPLASH_SCREEN_OPTION, SPLASH_SCREEN_OPTION_2);
		}
	...
	private void setAnimation(String category) {
		if (category.equals(SPLASH_SCREEN_OPTION_2)) {
			animation1();
